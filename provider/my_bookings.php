...
if (isset($_POST['btn_update_status'])) {
    ...
    if ($bookingId > 0 && in_array($status, ['pending', 'confirmed', 'completed', 'cancelled'])) {
        ...
        $stmt = $conn->prepare("UPDATE bookings SET status = ?, completion_date = ? WHERE id = ? AND provider_id = ?");
        if ($stmt) {
            $stmt->bind_param('ssii', $status, $completionDate, $bookingId, $providerId);
            if ($stmt->execute()) {
                            ...
                            <tr style="border-bottom: 1px solid var(--border);">
                                ...
                                <td style="padding: 10px;">
                                    ...
                                    <?php if ($booking['customer_phone']): ?>
                                        <div class="small"><?php echo htmlspecialchars($booking['customer_phone']); ?></div>
                                    <?php endif; ?>
                                </td>
                                <td style="padding: 10px;">
